...
*			Variance::operator<
*
*	Description:	Сравнение двух Variance на включение (x < y).
*	Parameters:	Variance& second - правая часть выражения
*	Returns:		1 если < и second - это Variance
*				-1 если > и second - это Variance
*				0 иначе (по сути, если параметры отличаются не более, чем на 0.01 или если second - это не Variance)
*	Throws:		-
*	Author:		dk
*	History:
*
...
double Variance::operator< (const ElemCondition& second) const {
    // пытаемся получить y типа Variance* из second типа el_axiom
    const Variance* y = dynamic_cast<const Variance*> (&second);
  
    // если y != NULL, т.н. second действительно имело тип Variance&, то тогда что-то проверяем
