...
*					RecognizerNet.cxx
*
*	Description:	Функции класса RecognizerNet - Реализация
*	Author:			dk
*	History:		
*
****************************************************************************/
											  ...
											  int numOfTest) const
{
	...
	for (int j = 0; j < (int) markUps.size(); j++) {
		// 1.1. подготовка сетей для распознвания
		Nets nets;
		nets.prepare(refs[j]);
		int resNum;
		// 1.2. поиск нештатных ситуаций непосредственно по markUps[j]
...
*	Description:	Запуск процесса распознавания
*	Parameters:	AxiomSetPop& pop - популяция, с которой работаем
*				precs - вектор содержащий параметры точности для каждой из аксиом
*	Returns:		0
*	Throws:		AxiomLibExeption - если популяция нулевого размера, или не инициализирована, или залочена
*	Author:		gevor
*	History:
