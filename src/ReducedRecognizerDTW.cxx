...
int ReducedRecognizerDTW::countDTWDist (const std::vector <int> &markup, const std::vector<int> &etalon, std::vector <double> &result) const {
	...
	for (int i = minLen-1; i < upTo; i++) {
        ...
        if (markup[i] == etalon[etLenMinusOne] || etalon[etLenMinusOne] == QUESTION_MARK_GLOBAL_INDEX)
			curVal = 0;
		else
			curVal = 1;
		...
		for (int k = etLen - 2; k >= 0; k--) {
            if (markup[i] != etalon[k] && etalon[k] != QUESTION_MARK_GLOBAL_INDEX)
				curVal++;
		...
		for (int k = i-1; k >= 0; k--) {
            if (markup[k] != etalon[etLenMinusOne] && etalon[etLenMinusOne] != QUESTION_MARK_GLOBAL_INDEX)
				curVal++;
		...
		for (int k = 1; k <= min(etLenMinusOne, i); k++) {
			...
			j = etLenMinusOne - k;
			for (l = i - k; l >= 0; l--) {
                if (markup[l] == etalon[j] || etalon[j] == QUESTION_MARK_GLOBAL_INDEX)
					curVal = 0;
				else
					curVal = 1;
...
//		}
	...
	for (int i = maxLen; i < len; i++) {
		...
		for (int k = maxLen - 2; k >= 0; k--) {
            if (markup[i - maxLenMinusOne + k] != etalon[etLenMinusOne] && etalon[etLenMinusOne] != QUESTION_MARK_GLOBAL_INDEX)
				curVal++;
			matrixErr[etLenMinusOne][k] = curVal;
		...
		for (int k = 1; k <= upTo; k++) {
			...
			for (l = maxLenMinusOne - k; l >= 0; l--) {
                if (markup[i - maxLenMinusOne + l] == etalon[j] || etalon[j] == QUESTION_MARK_GLOBAL_INDEX)
					curVal = 0;
				else
					curVal = 1;
				// вычисляем значение для всех возможных путей
			...
			l = maxLenMinusOne - k;
			for (j = etLenMinusOne - k - 1; j >= 0; j--) {
                if (markup[i - k] == etalon[j] || etalon[j] == QUESTION_MARK_GLOBAL_INDEX)
					curVal = 0;
				else
					curVal = 1;
				// вычисляем значение для всех возможных путей
